#!/bin/bash
# Debian Slim Surface - Setup Script
...
usage() {
    echo ""
    echo "Debian Slim Surface - Setup Script"
    echo ""
    echo "Usage: ./install.sh [command]"
    echo ""
    echo "Commands:"
    echo "  surface   Add linux-surface repo + install kernel (DO THIS FIRST!)"
    ...
    echo "  1. ./install.sh surface   # Add Surface repo + kernel, then REBOOT"
...
cmd_surface() {
    ...
    if dpkg -l | grep -q "linux-image-surface"; then
        log_ok "linux-image-surface already installed"
...
cmd_scan() {
    ...
    # Start check file
    cat > "$CHECKFILE" << EOF
# Debian Slim Surface - Package Check
...
EOF
    ...
    for pkg in linux-image-surface linux-headers-surface iptsd libwacom-surface; do
    ...
    for category in base shells editors network graphics browsers monitors dev utils cli_tools gui_apps; do
...
cmd_install() {
    ...
    if ! dpkg -l | grep -q "linux-image-surface"; then
        log_err "linux-image-surface not installed! Run: ./install.sh surface"
    fi
    ...
    if ! id diego &>/dev/null; then
        ...
        $SUDO useradd -m -G sudo,video,input -s /usr/bin/fish diego
